...
module Tax2018
  module Functions
    ...
    def get_annual_federal_tax(annual_income, married, dependents)
      ...
      if married
        brackets = Tax2018::Federal::INCOME_TAX_BRACKETS.fetch(:MARRIED)
      else
        brackets = Tax2018::Federal::INCOME_TAX_BRACKETS.fetch(:SINGLE)
      end
    ...
    def get_annual_state_tax(state_name, annual_income, married)
      ...
      if Tax2018::State::INCOME_TAX_BRACKETS.keys.include?(state_code.to_sym)
        if married
          brackets = Tax2018::State::INCOME_TAX_BRACKETS.fetch(state_code.to_sym).fetch(:MARRIED)
        else
          brackets = Tax2018::State::INCOME_TAX_BRACKETS.fetch(state_code.to_sym).fetch(:SINGLE)
        end
      else
        return
      end
    ...
    def annual_social_security_tax(annual_salary)
      [annual_salary * Tax2018::Federal::SocialSecurity::TAX_RATE,
       Tax2018::Federal::SocialSecurity::SALARY_CAP * Tax2018::Federal::SocialSecurity::TAX_RATE].min
    end

    def annual_medicare_tax(annual_salary)
      annual_salary * Tax2018::Federal::Medicare::TAX_RATE
    end
